...
function Setup() {
    ...
    $('.content-box ul.content-box-tabs li a').click( //When a tab is clicked...
        function () {
            ...
            $('.button_bottom').show(); //show bottom button
            return false;
        }
    );
    ...
    $(".nav-top-item").click(function () {
        $(".nav-top-item").each(function (index, item) {
    ...
    var href = window.location.pathname.toLowerCase() == "/admin" ? "/admin/category/" : window.location.pathname.toLowerCase();
    $(".nav-top-item").each(function (index, item) {
        $(item).removeClass("current");
    });
    ...
    $(".sub-menu-item").each(function (index, item) {
        ...
        var itemHref = $(item).attr("href").toLowerCase();

        if (href.indexOf(itemHref) != -1) {
            $(item).addClass("current");
            $(item).parent().parent().parent().find("a").first().addClass("current");
...
function GetWidthPopup() {
    return window.screen.width - 200;
}
function AlternateRow() {
    $('table.admin_management tbody tr').live("mouseover", function () {
        $(this).css('background-color', '#dff2fa');
    });
    $('table.admin_management tbody tr').live("mouseout", function () {
